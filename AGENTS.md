...
### 2026-07-19: 完成度声明必须基于源码事实，不能基于 PRD 标记

**事件**：执行 Wave 5 文档同步（W5-1）时发现，`README.md` 声称 PRD 全部 22 章节已完成（"[S1]-[S21] ✅ 已完成"），`docs/modules/INDEX.md` 全部 11 模块标 ✅，但 `spec.md`（2026-07-19 差距评估）通过 4 个并行子代理读取 100+ 源码文件逐项核实，发现实际加权完成度仅约 65%，多个模块存在致命 bug（M04 GROQ 漏字段、M05 工单非幂等、API 无 CSRF / 无限流 / 无文件类型白名单、Auth 密码重置流程断裂、Case Studies 残留、全站 i18n 硬编码、3 个 hooks 死代码、picsum.photos 违规等）。

**根因链**：
1. **依赖 PRD 标记而非源码核查** — PRD 章节 "✅ 已完成" 标记被当作事实，未对照源码验证
2. **README/INDEX 二手采信** — 文档之间互相引用，"PRD 标 ✅ → README 抄 ✅ → INDEX 抄 ✅" 形成循环引用，无源头事实校验
3. **"规则靠文件存在满足" 反模式** — 文件存在（如 `useRetry.ts`）被当作功能实现，不验证调用链；hook 实现完整但从未被任何组件使用
4. **测试覆盖偏差** — Vitest 49/49 + Playwright 47/47 全绿，但 M08-M11 零覆盖、8/10 UI 组件零覆盖、API 集成测试仅覆盖 2 个路由，"测试全绿" 不等于 "功能完整"
5. **MEMORY.md 缺失** — AGENTS.md 多处引用 `MEMORY.md` 作为真实状态源，但文件不存在，状态无单一事实源

**影响**：
- 用户基于 "100% 完成" 的错误预期做决策（部署、推广、销售）
- 实际部署到 https://www.techguru-it.asia 后多次出现 "已标完成功能无法使用 / 文案漏译 / 图片与内容不匹配" 等问题，损害品牌可信度
- 浪费 Wave 0-4 修复时间（本应在交付前发现，而非上线后用户发现）

**规则**：已写入第 36 条（强化） — 完成度声明必须基于源码事实，不能基于 PRD 标记或二手文档引用。具体强化要求：
- (a) 任何 "✅ 已完成" 声明必须可追溯到具体源码文件 + 行号或测试用例
- (b) README/INDEX 完成度表必须基于最新 spec.md 评估，不能直接抄 PRD 标记
- (c) 测试通过率 ≠ 功能完整度；必须同时报告 "覆盖率 + 测试数 + 模块覆盖范围"
- (d) MEMORY.md 作为单一真实状态源必须存在并维护（不能缺失）
- (e) 文档间互相引用时必须标注评估方法（如 "基于源码评估" vs "基于 PRD 标记"）

**关联规则**：#36（事实核查必须读源码）、#40（文档职责划分）、#41（文档同步检查清单）、#54（评估必须完整列出具体缺陷）、#55（PRD 必须反向同步）、#56（一次性穷极所有方面）
