...
async function testConnection() {
  try {
    ...
    console.log('Connection successful. Found posts:', result.length);
    return true;
  } catch (error) {
...
// 10 Blog articles - A series VMware (same as before, truncated for brevity)
const articles = [
  {
...
async function main() {
  ...
  if (!connected) {
    console.error('Cannot connect to Sanity. Exiting.');
    return;
  }
  
  console.log('\nStarting to publish 10 VMware blog articles...');
  console.log('---');
  ...
  console.log('\n---');
  console.log(`✅ Published ${successCount} out of 10 articles.`);
}
