...
const client = createClient({
  ...
  token: process.env.SANITY_API_TOKEN, // 需要在.env中配置
  useCdn: false,
});
...
async function seedSanity() {
  ...
  try {
    ...
    console.log(`   共写入 ${timelineEvents.length} 个时间轴事件\n`);
    ...
    console.log(`   共写入 ${teamMembers.length} 个团队成员\n`);
    ...
    console.log(`   共写入 ${qualifications.length} 个资质\n`);

    console.log('✅ 所有数据已成功写入Sanity CMS！');
    console.log('\n下一步：');
