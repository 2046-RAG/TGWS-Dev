...
// Expanded articles that were below 2000 words
const articles = [];

// Article 1: vmware-cloud-foundation-private-cloud (1838 -> 2000+)
articles.push({
...
// Article 2: nsx-t-enterprise-deployment (1894 -> 2000+)
articles.push({
...
// Article 4: veeam-backup-vmware-best-practice (1485 -> 2000+)
articles.push({
...
// Article 5: vmware-vsphere-8-upgrade-lessons (1639 -> 2000+)
articles.push({
...
// Article 6: vmware-ha-vs-ft-which-need (1745 -> 2000+)
articles.push({
...
Let us put real numbers on availability. Here is what different availability targets mean in terms of annual downtime:

- **99.9% (three nines):** 8 hours 46 minutes per year
- **99.99% (four nines):** 52 minutes per year
- **99.999% (five nines):** 5 minutes 15 seconds per year

HA alone typically delivers 99.9% availability. That means 8+ hours of potential downtime per year from host failures. For most workloads, that is acceptable.

FT delivers 99.999% availability for the protected VM. That means less than 5 minutes of downtime per year. But remember -- FT only protects against host failure. It does not protect against storage failure, network failure, or application crashes.

For true five-nines availability, you need:

1. FT for host-level protection (zero-downtime failover)
2. Redundant storage (vSAN FTT=2 or mirrored SAN)
3. Redundant networking (multi-path, bonded NICs)
4. Application-level clustering (SQL Always On, Oracle RAC)
5. Load balancing (NSX ALB, F5)

That is a lot of infrastructure for five nines. Most enterprises settle for four nines (99.99%) with HA and redundant storage.
...
## Advanced HA Features You Should Know

**vSphere HA Orchestrated Restart.** When a host fails, HA restarts VMs in dependency order. Database VMs start before application servers, which start before web servers. That prevents cascading restart failures where an app tries to connect to a database that has not started yet.

**Proactive HA.** vSphere monitors hardware health (CPU temperature, memory errors, disk failures). If hardware degrades but has not failed yet, Proactive HA migrates VMs away from the unhealthy host before it fails. We enable this on every cluster.

**HA Admission Control.** This ensures enough resources exist to restart all VMs if a host fails. Without it, HA might not have enough resources. We set admission control to reserve 33% of cluster resources (one host's worth).

**Datastore Heartbeating.** When a host loses network connectivity, HA uses datastore heartbeats to determine if the host is actually dead or just isolated. If the host can still write to the datastore, HA knows it is alive and avoids unnecessary restarts.
...
**Q: What is the difference between HA and VMware SRM?**
A: HA protects against host failure within a single cluster (VMs restart on other hosts in the same site). SRM protects against site failure (VMs failover to a completely different data center). You need both for comprehensive protection.`
});
...
async function main() {
  console.log(`Expanding ${articles.length} articles to 2000+ words`);
  for (const article of articles) {
    await processArticle(article);
  }
  console.log('\n=== SUMMARY ===');
  console.log(`Expanded: ${articles.length} articles`);
